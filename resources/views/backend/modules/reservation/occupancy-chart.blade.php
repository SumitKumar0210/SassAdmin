...
<body>
	<section>
		...
		<h4 class="heading2">Room Occupancy Chart ( {{date('d/m/Y')}} )</h4>
		...
		<table style="width:100%;text-align: center;" class="invoice_table">
			<tr>
				...
				<th style="text-align: center;">Room Tariff</th>
			...
			@php
				$room_type = App\Models\RoomType::where('id',$res_room->room_category_id)->value('room_category');
				$company_name = App\Models\Reservation::where('id',$res_room->reservation_id)->value('company_name');
				$advance = App\Models\AdvanceAmount::where('reservation_id',$res_room->reservation_id)->value('amount');
			@endphp
			<tr>
				<td>{{$n++}}</td>
				<td>{{$res_room->roomData->room_number}}</td>
				<td>{{$room_type}}</td>
				<td>{{$res_room->amount}}</td>
				...
				<td>{{date('d-m-Y',strtotime($res_room->checkedin_at))}}</td>
				<td>{{date('h:i A',strtotime($res_room->checkedin_at))}}</td>
				<td>{{$res_room->daystay}}</td>
				<td>{{$advance ?? 0}}</td>
			</tr>
			@endforeach
		</table>
