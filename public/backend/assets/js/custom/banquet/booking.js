...
function collectAllData(){
    ...
    $('#banquet_eventDate').val(currentDate);
    ...
    if(availableHalls.length <= 0){
        ...
        availableHalls.forEach(function(hall) {
            $('.hall_detail').append(`<div class="col-md-4 mb-3 border-primary border-3" onClick="addHall(${hall.id},this)">
                <div class="item-hall-type border rounded-3 p-3">
...
function createBooking(x = 0){
    ...
    let adjustment = $('#banquet_adjustment').val() || 0;
    if(client == '' || eventDate == '' || startTime == '' || endTime == '' || eventType == '' || guestCount == '' || phone == '' || hall_charge == '' || hall_discount == '' || hall_total == '' || complimentary_room == '' || hall_extra_room == '' || hall_per_room_capacity == '' || hall_per_room_charge == '' || grand_total_hall_charge == '' || total_food_charge == '' || consumable_charge == '' || total_extra_room_charge == ''){
