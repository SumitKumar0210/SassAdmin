...
class SingleBookingSaleController extends Controller
{
    public function index(){
        $hotlr = HotlrConfiguration::get(['logo','name']);
        return view('backend.modules.report.single_booking_sale_report',compact('hotlr'));
    }

    public function singleBookingSaleReportView(Request $request){
        ...
        $resRoom = Invoice::where('booking_type','Single')->whereBetween('invoice_date',[$dateFrom,$dateTo])->get();
        $total_cash = 0;
        $total_card = 0;
        $total_amount = 0;
        ...
        ->addColumn('room_type',function($row){
            $category = InvoiceRoomDetail::where('invoice_id',$row->id)->value('room_type');
            return $category;
        })
        ->addColumn('room_number',function($row){
            $category = InvoiceRoomDetail::where('invoice_id',$row->id)->value('room_number');
            return $category;
        })
